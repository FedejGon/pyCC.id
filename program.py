...
#    {'constraint': 'f1 odd', 'penalty': 1e1, 'eval': 'array','Nval_array':100}, # penaly is optional
#    {'constraint': 'f2 odd', 'penalty': 1e1, 'eval': 'array','Nval_array':100}, # eval=data/array array is default
#]


#constraints=None
# it is generally better to increase penalty than adding more Nval values becasue the latter increase computational cost as cost functions is evaluated at each point.

models,evals,params = pycc.train_hybrid(
    #df, equation, constraints=constraints,
    df, equation,
    neurons=100, lr=1e-4, epochs=20000, error_threshold=1e-6, extrapolation=None,weight_loss_param=1e1
    #param_penalty_weight=0.0
    )
    #lambda_penalty=1.0, apply_restriction=True
#)

#models,evals = pycc.train(
#    df, equation, constraints=constraints,
#    neurons=100, lr=1e-3, epochs=5000, error_threshold=1e-6, extrapolation=None
#    )
