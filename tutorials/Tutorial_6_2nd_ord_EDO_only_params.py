...
# This tutorial shows the comparison of identifying a second order ODE using 
# only parameter approach with NN and Poly methods.
# As a conclusion, we see that Poly method is computationally more efficient


# Integration of the theoretical EDO
# To this aim, we may use standard python integrators, but in this example we
# will show how to use the pycc library to integrate the theoretical ODE.
#
# define parameters
...
models, evals, obtained_coefs = pycc.train(df, eqs,method='Poly', params=params_poly)
# Print learned parameters (if any)
if obtained_coefs:
    print("\nLearned scalar parameters:")
    for name, val in obtained_coefs.items():
